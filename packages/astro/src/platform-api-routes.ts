...
function setPlatformCookies(
  ...
  if (user) {
    ...
    cookies.set(COOKIE_AUTH_SESSION, JSON.stringify({ user, expiresAt }), {
      httpOnly: false,
      secure: isProduction,
      sameSite: "lax",
      path: "/",
...
function clearPlatformCookies(cookies: AstroCookies): void {
...
export function createPlatformAuthRoutes(config: InAIAuthConfig = {}) {
  ...
  async function handleLogin(context: AstroAPIContext): Promise<Response> {
    try {
      ...
      setPlatformCookies(context.cookies, tokens, result.user, { isNewSession: true });
  ...
  async function handleRefresh(context: AstroAPIContext): Promise<Response> {
    try {
      if (isSessionExpired(context.cookies)) {
        clearPlatformCookies(context.cookies);
      ...
      setPlatformCookies(context.cookies, tokens, user);
  ...
  async function handleLogout(context: AstroAPIContext): Promise<Response> {
    try {
      const accessToken = context.cookies.get(COOKIE_AUTH_TOKEN)?.value;
      if (accessToken) {
  ...
  async function handleMe(context: AstroAPIContext): Promise<Response> {
    try {
      const accessToken = context.cookies.get(COOKIE_AUTH_TOKEN)?.value;
      if (!accessToken) {
  ...
  async function handleRegister(context: AstroAPIContext): Promise<Response> {
    try {
      ...
      return jsonResponse({ user: result.user, tenant: result.tenant });
    } catch (err) {
